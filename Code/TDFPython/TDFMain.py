...
def compute_persistence_2DImg_1DHom(f):
    ...
    persistence_result = cubePers(np.reshape(
        f_padded, f_padded.size).tolist(), list(f_padded.shape), 0.001)     #0.001 is precision paramameter
...
def compute_dgm_force(lh_dgm, gt_dgm):
    ...
    # check to ensure that all gt dots have persistence 1 ie all points are meaningful holes
    tmp = gt_pers > 0.999
...
def compute_topological_loss(lh_dgm, gt_dgm):
    ...
    # For each index, adds the squared forces (displacements) in the x and y directions to the loss. 
    # This essentially sums the squared Euclidean distances from the learned homology points to their 
    # target positions (0, 1).

    for idx in idx_holes_to_remove:
        loss = loss + force_list[idx, 0] ** 2 + force_list[idx, 1] ** 2
    
    # For each index, adds the squared forces in the x and y directions to the loss. 
    # This sums the squared Euclidean distances that move the unwanted points toward the diagonal.
...
def save_pers_dgms(fig_fname, lh_dgm, lh_dname, gt_dgm, gt_dname):
    ...
    # Plots the learned homology persistence points in green with the label lh_dname.
    plt.scatter([lh_dgm[:, 0]], [lh_dgm[:, 1]], color='green', label=lh_dname)

    # For each hole to fix, adds a red arrow from the learned point to its target position. 
    # The arrow indicates the direction and magnitude of the force.
    for idx in idx_holes_to_fix:
        plt.arrow(lh_dgm[idx, 0], lh_dgm[idx, 1], force_list[idx, 0],
                  force_list[idx, 1], head_width=0.02, head_length=0.03,
                  color='r')

    # For each hole to remove, adds a cyan arrow from the learned point towards the diagonal.
    for idx in idx_holes_to_remove:
        plt.arrow(lh_dgm[idx, 0], lh_dgm[idx, 1], force_list[idx, 0],
                  force_list[idx, 1], head_width=0.02, head_length=0.03,
                  color='c')
...
def draw_critical_pts(fig_fname, lh_img, lh_dgm, lh_bcp, lh_dcp, gt_dgm):   # lh_img = likelihood
    """
    ...
    # For each index, plot the bcp with a red triangle (v) & the dcp with a red circle (o). 
    # The label includes the force value indicating how much the point needs to be adjusted
    for idx in idx_holes_to_fix:
    ...
    # For each index, plot the bcp with a cyan triangle (v) & the dcp with a cyan circle (o).
    for idx in idx_holes_to_remove:
...
if __name__ == "__main__":
    ...
    plt.imsave(result_folder + 'groundtruth_cropped.png', groundtruth, cmap='gray')
